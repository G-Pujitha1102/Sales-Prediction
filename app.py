...
st.markdown("""
    ...
    .block-container {
        padding-top: 140px !important; /* Added proper value */
    }
...
# Create a copy of the original data and add prediction columns
export_df = df.copy()
export_df['LR_Prediction'] = lr_pred
...
lr_metrics = calculate_metrics(y, lr_pred)
rf_metrics = calculate_metrics(y, rf_pred)
...
if menu == "Prediction":
    ...
    # Determine which model performs better based on R² score
    ...
    if submit:
        ...
        # Categorize prediction level based on average of both models
        avg_pred = (lr_result + rf_result) / 2
        if avg_pred < 10:
            st.warning("📉 Predicted Sales are **LOW**. Consider increasing your advertising budget.")
        elif 10 <= avg_pred <= 20:
            st.info("📊 Predicted Sales are **AVERAGE**. You might want to optimize your campaigns.")
        else:
            st.success("🚀 Predicted Sales are **HIGH**! Great job on your marketing efforts.")
...
# -----------------------
    ...
    metrics_df = pd.DataFrame({
        ...
        "Linear Regression": [
            r2_score(y, lr_pred),
            np.sqrt(mean_squared_error(y, lr_pred)),
            mean_absolute_error(y, lr_pred)
        ],
        "Random Forest": [
            r2_score(y, rf_pred),
    ...
    for metric in metrics:
        # Select row for the current metric and columns for models
